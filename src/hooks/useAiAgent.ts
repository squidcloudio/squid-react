    ...
    () => {
      ...
      if (file) {
        if (options?.voiceOptions) {
          return from(squid.ai().agent(agentId).transcribeAndAskWithVoiceResponse(file, options)).pipe(
          ...
          return from(squid.ai().agent(agentId).transcribeAndChat(file, options)).pipe(
      ...
      // (c) Text + Voice Response
        ...
        if (options?.voiceOptions) {
          return from(squid.ai().agent(agentId).askWithVoiceResponse(prompt, options)).pipe(
